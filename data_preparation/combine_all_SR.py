...
if not sr_files:
    ...
    print(
        "  - Make sure you're in the correct directory with SR2014.csv, SR2015.csv, etc."
    )
    exit(1)
...
for i, file in enumerate(sr_files_sorted):
    ...
    try:
        ...
        print("      - Parser error detected, trying with error handling...")
        try:
            ...
            combined_data.append(df)
            print(
                f"      - Loaded {rows_in_file:,} rows (some malformed lines skipped)"
            )

        except Exception as e2:
            print(f"      - Still failed with error handling: {e2}")
            print("      - Trying manual column inspection...")
            ...
            try:
                ...
                if len(columns) > 9:
                    ...
                    combined_data.append(df)
                    print(
                        f"      - Loaded {rows_in_file:,} rows using first 9 columns only"
                    )
                else:
                    print("      - Could not resolve column mismatch")
                    continue
...
try:
    ...
    final_df["Creation Date"] = pd.to_datetime(
        final_df["Creation Date"], errors="coerce"
    )
    ...
    print("  - Files will be combined in filename order")
