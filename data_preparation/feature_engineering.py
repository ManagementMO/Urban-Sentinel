...
class WardBasedModelDataGeneratorCSV:
    ...
    def clean_and_prepare_data(self) -> bool:
        ...
        self.df = self.df.dropna(
            subset=["Creation Date", "Ward", "Service Request Type"]
        )

        # Filter to valid years
        valid_years = range(
            self.config["history_start_year"], self.config["target_year"] + 1
        )
        ...
        if len(self.df) == 0:
            print(
                "❌ ERROR: No data remaining after cleaning. Check your date range and data quality."
            )
            return False

        print(
            f"   ✓ Cleaned data: {len(self.df):,} records (removed {initial_count - len(self.df):,})"
        )
        ...
        if self.wards_gdf is not None:
            ...
            if self.ward_name_col is None:
                ...
                self.wards_gdf[self.ward_name_col] = (
                    self.wards_gdf[self.ward_name_col]
                    .str.strip()
                    .str.title()
                    .str.replace(r"\s+", " ", regex=True)
    ...
    def create_advanced_features(self) -> bool:
        ...
        if self.wards_gdf is not None:
            ...
            if len(matched_wards) < len(service_wards) * 0.8:
                print(
                    f"   ⚠️  Low match rate: {len(matched_wards)}/{len(service_wards)}"
                )
            ...
            print(
                f"   ✓ Processing all {len(service_wards)} wards from service requests"
            )
        ...
        print(
            f"   ✓ Created {len(self.features_df.columns)} features for {len(self.features_df)} wards"
        )
    ...
    def _create_ward_features(self, ward: str, ward_data: pd.DataFrame) -> Dict:
        ...
        features.update(
            self._create_categorical_blight_features(historical_data, target_data)
        )

        return features

    def _create_volume_features(
        self, historical_data: pd.DataFrame, target_data: pd.DataFrame
    ) -> Dict:
        ...
        for year in range(
            self.config["history_start_year"], self.config["history_end_year"] + 1
        ):
        ...
        if len(yearly_counts) > 0:
            ...
            features["requests_std_per_year"] = (
                yearly_counts.std() if len(yearly_counts) > 1 else 0
            )
            features["requests_min_per_year"] = yearly_counts.min()
            features["requests_max_per_year"] = yearly_counts.max()
            features["requests_range_per_year"] = (
                yearly_counts.max() - yearly_counts.min()
            )
        else:
        ...
        if len(yearly_counts) > 1:
            ...
            features["requests_recent_vs_early_ratio"] = (
                recent_avg / early_avg if early_avg > 0 else 0
            )
        else:
    ...
    def _create_blight_features(
        ...
        blight_data = historical_data[
            historical_data["Service Request Type"].isin(
                list(self.blight_indicators.keys())
            )
        ]

        # Target blight data
        target_blight = target_data[
            target_data["Service Request Type"].isin(
                list(self.blight_indicators.keys())
            )
        ]
        ...
        features["blight_rate"] = (
            len(blight_data) / len(historical_data) if len(historical_data) > 0 else 0
        )
        features["target_blight_rate"] = (
            len(target_blight) / len(target_data) if len(target_data) > 0 else 0
        )
        ...
        if len(yearly_blight_counts) > 0:
            ...
            features["blight_std_per_year"] = (
                yearly_blight_counts.std() if len(yearly_blight_counts) > 1 else 0
            )
            features["blight_min_per_year"] = yearly_blight_counts.min()
            features["blight_max_per_year"] = yearly_blight_counts.max()
            features["blight_range_per_year"] = (
                yearly_blight_counts.max() - yearly_blight_counts.min()
            )
        else:
        ...
        if len(yearly_blight_counts) > 1:
            ...
            early_blight_avg = (
                yearly_blight_counts[early_years].mean() if early_years else 0
            )
            recent_blight_avg = (
                yearly_blight_counts[recent_years].mean() if recent_years else 0
            )

            features["blight_early_period_avg"] = early_blight_avg
            features["blight_recent_period_avg"] = recent_blight_avg
            features["blight_recent_vs_early_ratio"] = (
                recent_blight_avg / early_blight_avg if early_blight_avg > 0 else 0
            )
        else:
        ...
        yearly_blight_types = blight_data.groupby("year")[
        ...
        features["blight_intensity_avg"] = (
            total_weighted_score / len(blight_data) if len(blight_data) > 0 else 0
        )
    ...
    def _create_temporal_features(self, historical_data: pd.DataFrame) -> Dict:
        ...
        for year in range(
            ...
            if len(year_data) > 0:
                ...
                features[f"weekday_requests_{year}"] = len(
                    year_data[year_data["day_of_week"] < 5]
                )
                features[f"weekend_requests_{year}"] = len(
                    year_data[year_data["day_of_week"] >= 5]
                )
                ...
                for day_num, day_name in enumerate(
                    ...
                    features[f"{day_name}_requests_{year}"] = len(
                        year_data[year_data["day_of_week"] == day_num]
                    )

                # Monthly patterns by year
                for month in range(1, 13):
                    features[f"month_{month}_requests_{year}"] = len(
                        year_data[year_data["month"] == month]
                    )

                # Quarterly patterns by year
                for quarter in range(1, 5):
                    features[f"quarter_{quarter}_requests_{year}"] = len(
                        year_data[year_data["quarter"] == quarter]
                    )
                ...
                if "Creation Date" in year_data.columns:
                    ...
                    features[f"morning_requests_{year}"] = len(
                        year_data_copy[year_data_copy["hour"].between(6, 11)]
                    )
                    features[f"afternoon_requests_{year}"] = len(
                        year_data_copy[year_data_copy["hour"].between(12, 17)]
                    )
                    features[f"evening_requests_{year}"] = len(
                        year_data_copy[year_data_copy["hour"].between(18, 23)]
                    )
                    features[f"night_requests_{year}"] = len(
                        year_data_copy[~year_data_copy["hour"].between(6, 23)]
                    )
                    features[f"peak_hours_requests_{year}"] = len(
                        year_data_copy[year_data_copy["hour"].between(9, 17)]
                    )

                    # Hour-by-hour breakdown by year
                    for hour in range(24):
                        features[f"hour_{hour}_requests_{year}"] = len(
                            year_data_copy[year_data_copy["hour"] == hour]
                        )
                else:
                    features[f"morning_requests_{year}"] = 0
                    features[f"afternoon_requests_{year}"] = 0
        ...
        if total > 0:
            features["temporal_weekday_rate_overall"] = (
                len(historical_data[historical_data["day_of_week"] < 5]) / total
            )
            features["temporal_weekend_rate_overall"] = (
                len(historical_data[historical_data["day_of_week"] >= 5]) / total
            )

            # Most active day/month/quarter overall
            day_counts = historical_data["day_of_week"].value_counts()
            features["most_active_day_overall"] = (
                day_counts.idxmax() if len(day_counts) > 0 else 0
            )
            features["most_active_day_count_overall"] = (
                day_counts.max() if len(day_counts) > 0 else 0
            )

            month_counts = historical_data["month"].value_counts()
            features["most_active_month_overall"] = (
                month_counts.idxmax() if len(month_counts) > 0 else 0
            )
            features["most_active_month_count_overall"] = (
                month_counts.max() if len(month_counts) > 0 else 0
            )

            quarter_counts = historical_data["quarter"].value_counts()
            features["most_active_quarter_overall"] = (
                quarter_counts.idxmax() if len(quarter_counts) > 0 else 0
            )
            features["most_active_quarter_count_overall"] = (
                quarter_counts.max() if len(quarter_counts) > 0 else 0
            )
        else:
    ...
    def _create_diversity_features(self, historical_data: pd.DataFrame) -> Dict:
        ...
        for year in range(
            ...
            if len(year_data) > 0:
                ...
                if len(service_counts) > 0:
                    ...
                    service_entropy = -np.sum(
                        service_probs * np.log2(service_probs + 1e-10)
                    )
                    features[f"service_type_diversity_{year}"] = service_entropy

                    # Top service dominance by year
                    features[f"top_service_dominance_{year}"] = service_counts.iloc[
                        0
                    ] / len(year_data)
                    ...
                    for i in range(1, 11):
                        if i <= len(service_counts):
                            features[f"top_{i}_service_count_{year}"] = (
                                service_counts.iloc[i - 1]
                            )
                            features[f"top_{i}_service_rate_{year}"] = (
                                service_counts.iloc[i - 1] / len(year_data)
                            )
                        else:
                            features[f"top_{i}_service_count_{year}"] = 0
                            features[f"top_{i}_service_rate_{year}"] = 0
                ...
                if "Division" in year_data.columns:
                    ...
                    if len(division_counts) > 0:
                        ...
                        division_entropy = -np.sum(
                            division_probs * np.log2(division_probs + 1e-10)
                        )
        ...
        if len(service_counts_overall) > 0:
            service_probs_overall = (
                service_counts_overall / service_counts_overall.sum()
            )
            features["service_type_diversity_overall"] = -np.sum(
                service_probs_overall * np.log2(service_probs_overall + 1e-10)
            )
            features["top_service_dominance_overall"] = service_counts_overall.iloc[
                0
            ] / len(historical_data)
            ...
            for i in range(1, 11):
                if i <= len(service_counts_overall):
                    features[f"top_{i}_service_count_overall"] = (
                        service_counts_overall.iloc[i - 1]
                    )
                    features[f"top_{i}_service_rate_overall"] = (
                        service_counts_overall.iloc[i - 1] / len(historical_data)
                    )
                else:
        ...
        if "Division" in historical_data.columns:
            ...
            if len(division_counts_overall) > 0:
                division_probs_overall = (
                    division_counts_overall / division_counts_overall.sum()
                )
                features["division_diversity_overall"] = -np.sum(
                    division_probs_overall * np.log2(division_probs_overall + 1e-10)
                )
            else:
    ...
    def _create_trend_features(self, historical_data: pd.DataFrame) -> Dict:
        ...
        yearly_blight_counts = (
            blight_data.groupby("year").size()
        ...
        for year in range(
            ...
            if year > self.config["history_start_year"]:
                ...
                features[f"requests_year_over_year_change_{year}"] = (
                    current_requests - prev_year_requests
                )
                features[f"blight_year_over_year_change_{year}"] = (
                    current_blight - prev_year_blight
                )

                # Growth rate
                if prev_year_requests > 0:
                    features[f"requests_growth_rate_{year}"] = (
                        current_requests - prev_year_requests
                    ) / prev_year_requests
                else:
                    features[f"requests_growth_rate_{year}"] = 0
                ...
                if prev_year_blight > 0:
                    features[f"blight_growth_rate_{year}"] = (
                        current_blight - prev_year_blight
                    ) / prev_year_blight
                else:
                    features[f"blight_growth_rate_{year}"] = 0
        ...
        if len(yearly_counts) > 1:
            ...
            features["trend_overall_r2"] = (
                np.corrcoef(years, counts)[0, 1] ** 2 if len(years) > 1 else 0
            )
        ...
        if len(yearly_blight_counts) > 1:
            ...
            # Blight trend
            features["trend_blight_slope"] = np.polyfit(blight_years, blight_counts, 1)[
                0
            ]
            features["trend_blight_r2"] = (
                np.corrcoef(blight_years, blight_counts)[0, 1] ** 2
            )

            # Blight trend acceleration
            if len(blight_counts) > 2:
                features["trend_blight_acceleration"] = np.polyfit(
                    blight_years, blight_counts, 2
                )[0]
            else:
        ...
        if len(yearly_counts) > 1:
            ...
            features["trend_recent_vs_early"] = (
        ...
        for year in range(
            ...
            year_quarterly = quarterly_counts[
                quarterly_counts.index.get_level_values(0) == year
            ]
            ...
            year_seasonal = seasonal_counts[
                seasonal_counts.index.get_level_values(0) == year
            ]
        ...
        if len(yearly_counts) > 2:
            counts_values = [
                features[f"requests_count_{year}"]
                for year in range(
                    self.config["history_start_year"],
                    self.config["history_end_year"] + 1,
                )
            ]
            features["peak_year"] = self.config["history_start_year"] + np.argmax(
                counts_values
            )
            features["trough_year"] = self.config["history_start_year"] + np.argmin(
                counts_values
            )
            features["peak_to_trough_ratio"] = (
                np.max(counts_values) / np.min(counts_values)
                if np.min(counts_values) > 0
                else 1.0
            )
        else:
        ...
        if len(yearly_counts) >= 3:
            ...
            recent_3_year_avg = np.mean(
                [
                    features[f"requests_count_{year}"]
                    for year in range(
                        self.config["history_end_year"] - 2,
                        self.config["history_end_year"] + 1,
                    )
                ]
            )
            ...
            features["momentum_indicator"] = (
                recent_3_year_avg / overall_avg if overall_avg > 0 else 1.0
            )
        else:
    ...
    def _create_seasonal_features(self, historical_data: pd.DataFrame) -> Dict:
        ...
        for season in ["winter", "spring", "summer", "fall"]:
            ...
            features[f"seasonal_{season}_rate"] = (
                len(seasonal_data) / total if total > 0 else 0
            )
        ...
        for season in ["winter", "spring", "summer", "fall"]:
            ...
            if len(yearly_seasonal_counts) > 0:
                ...
                features[f"seasonal_{season}_std"] = (
                    yearly_seasonal_counts.std()
                    if len(yearly_seasonal_counts) > 1
                    else 0
                )
                features[f"seasonal_{season}_consistency"] = (
                    1 / (yearly_seasonal_counts.std() + 1)
                    if yearly_seasonal_counts.std() > 0
                    else 1
                )
            else:
    ...
    def _create_ratio_features(self, historical_data: pd.DataFrame) -> Dict:
        ...
        infrastructure_count = len(
            historical_data[
                historical_data["Service Request Type"].str.contains(
                    "|".join(infrastructure_types), na=False
                )
            ]
        )
        quality_count = len(
            historical_data[
                historical_data["Service Request Type"].str.contains(
                    "|".join(quality_types), na=False
                )
            ]
        )

        features["ratio_blight_to_total"] = blight_count / total if total > 0 else 0
        features["ratio_infrastructure_to_total"] = (
            infrastructure_count / total if total > 0 else 0
        )
        ...
        for year in range(
            ...
            if year_total > 0:
                year_blight = len(
                    year_data[
                        year_data["Service Request Type"].isin(
                            list(self.blight_indicators.keys())
                        )
                    ]
                )
                year_infrastructure = len(
                    year_data[
                        year_data["Service Request Type"].str.contains(
                            "|".join(infrastructure_types), na=False
                        )
                    ]
                )
                ...
                features[f"ratio_infrastructure_to_total_{year}"] = (
                    year_infrastructure / year_total
                )
        ...
        for category in ["blight", "infrastructure", "quality"]:
            ...
            for year in range(
                ...
                yearly_ratios[category].append(
                    features[f"ratio_{category}_to_total_{year}"]
                )
            ...
            if len(yearly_ratios[category]) > 1:
                ...
                features[f"ratio_{category}_consistency"] = (
                    1 / (ratios_series.std() + 1) if ratios_series.std() > 0 else 1
                )
            else:
    ...
    def _create_statistical_features(self, historical_data: pd.DataFrame) -> Dict:
        ...
        for year in range(
            ...
            if len(year_data) > 0:
                ...
                if len(monthly_counts) > 0:
                    ...
                    features[f"monthly_std_{year}"] = (
                        monthly_counts.std() if len(monthly_counts) > 1 else 0
                    )
                    features[f"monthly_skewness_{year}"] = (
                        monthly_counts.skew() if len(monthly_counts) > 2 else 0
                    )
                    features[f"monthly_kurtosis_{year}"] = (
                        monthly_counts.kurtosis() if len(monthly_counts) > 3 else 0
                    )
                    features[f"monthly_cv_{year}"] = (
                        monthly_counts.std() / monthly_counts.mean()
                        if monthly_counts.mean() > 0
                        else 0
                    )
                    features[f"monthly_range_{year}"] = (
                        monthly_counts.max() - monthly_counts.min()
                    )
                    features[f"monthly_iqr_{year}"] = monthly_counts.quantile(
                        0.75
                    ) - monthly_counts.quantile(0.25)
                else:
                    features[f"monthly_mean_{year}"] = 0
                    features[f"monthly_std_{year}"] = 0
                    features[f"monthly_skewness_{year}"] = 0
                ...
                if len(quarterly_counts) > 0:
                    ...
                    features[f"quarterly_std_{year}"] = (
                        quarterly_counts.std() if len(quarterly_counts) > 1 else 0
                    )
                    features[f"quarterly_range_{year}"] = (
                        quarterly_counts.max() - quarterly_counts.min()
                    )
                else:
                    features[f"quarterly_mean_{year}"] = 0
                    features[f"quarterly_std_{year}"] = 0
                ...
                if "Creation Date" in year_data.columns:
                    year_data_copy = year_data.copy()
                    year_data_copy["week"] = (
                        year_data_copy["Creation Date"].dt.isocalendar().week
                    )
                    weekly_counts = year_data_copy.groupby("week").size()

                    if len(weekly_counts) > 0:
                        features[f"weekly_mean_{year}"] = weekly_counts.mean()
                        features[f"weekly_std_{year}"] = (
                            weekly_counts.std() if len(weekly_counts) > 1 else 0
                        )
                        features[f"weekly_range_{year}"] = (
                            weekly_counts.max() - weekly_counts.min()
                        )
                    else:
                        features[f"weekly_mean_{year}"] = 0
                        features[f"weekly_std_{year}"] = 0
        ...
        monthly_std = (
            monthly_counts_overall.std() if len(monthly_counts_overall) > 1 else 0
        )
        monthly_cv = (
            monthly_std / monthly_counts_overall.mean()
            if monthly_counts_overall.mean() > 0
            else 0
        )

        peak_month_ratio = (
            monthly_counts_overall.max() / monthly_counts_overall.mean()
            if monthly_counts_overall.mean() > 0
            else 0
        )
        ...
        if len(service_counts) > 0:
            features["service_type_entropy"] = -np.sum(
                (service_counts / service_counts.sum())
                * np.log2(service_counts / service_counts.sum() + 1e-10)
            )
            features["service_type_gini"] = 1 - np.sum(
                (service_counts / service_counts.sum()) ** 2
            )
            features["service_type_max_share"] = (
                service_counts.max() / service_counts.sum()
            )
            features["service_type_min_share"] = (
                service_counts.min() / service_counts.sum()
            )
        else:
        ...
        if "Creation Date" in historical_data.columns:
            ...
            if len(hour_counts) > 0:
                features["hour_concentration_entropy"] = -np.sum(
                    (hour_counts / hour_counts.sum())
                    * np.log2(hour_counts / hour_counts.sum() + 1e-10)
                )
                features["hour_concentration_gini"] = 1 - np.sum(
                    (hour_counts / hour_counts.sum()) ** 2
                )
        ...
        if len(dow_counts) > 0:
            features["dow_concentration_entropy"] = -np.sum(
                (dow_counts / dow_counts.sum())
                * np.log2(dow_counts / dow_counts.sum() + 1e-10)
            )
            features["dow_concentration_gini"] = 1 - np.sum(
                (dow_counts / dow_counts.sum()) ** 2
            )
        ...
        if len(season_counts) > 0:
            features["season_concentration_entropy"] = -np.sum(
                (season_counts / season_counts.sum())
                * np.log2(season_counts / season_counts.sum() + 1e-10)
            )
            features["season_concentration_gini"] = 1 - np.sum(
                (season_counts / season_counts.sum()) ** 2
            )
        ...
        if len(blight_data) > 0:
            ...
            if len(blight_monthly) > 0:
                ...
                features["blight_monthly_cv"] = (
                    blight_monthly.std() / blight_monthly.mean()
                    if blight_monthly.mean() > 0
                    else 0
                )
                features["blight_monthly_skewness"] = (
                    blight_monthly.skew() if len(blight_monthly) > 2 else 0
                )
                features["blight_monthly_kurtosis"] = (
                    blight_monthly.kurtosis() if len(blight_monthly) > 3 else 0
                )
            else:
    ...
    def _create_categorical_blight_features(
        self, historical_data: pd.DataFrame, target_data: pd.DataFrame
    ) -> Dict:
        ...
        if len(blight_data) > 0:
            ...
            features["overall_most_common_blight"] = (
                overall_most_common.iloc[0] if len(overall_most_common) > 0 else "None"
            )
        else:
        ...
        if len(target_blight_data) > 0:
            ...
            features["recent_most_common_blight"] = (
                recent_most_common.iloc[0] if len(recent_most_common) > 0 else "None"
            )
        else:
        ...
        if len(blight_data) > 0:
            recent_years = [
                self.config["history_end_year"] - 1,
                self.config["history_end_year"],
            ]
            recent_historical_blight = blight_data[
                blight_data["year"].isin(recent_years)
            ]

            if len(recent_historical_blight) > 0:
                recent_historical_most_common = recent_historical_blight[
                    "Service Request Type"
                ].mode()
                features["recent_historical_most_common_blight"] = (
                    recent_historical_most_common.iloc[0]
                    if len(recent_historical_most_common) > 0
                    else "None"
                )
            else:
        ...
        unique_blight_types = (
            blight_data["Service Request Type"].nunique() if len(blight_data) > 0 else 0
        )
        ...
        if len(blight_data) > 0:
            infrastructure_count = len(
                blight_data[
                    blight_data["Service Request Type"].str.contains(
                        "Road|Sidewalk|Catch Basin", na=False
                    )
                ]
            )
            visual_count = len(
            ...
            sanitation_count = len(
                blight_data[
                    blight_data["Service Request Type"].str.contains(
                        "Litter|Dumping|Garbage", na=False
                    )
                ]
            )
            property_count = len(
                blight_data[
                    blight_data["Service Request Type"].str.contains(
                        "Grass|Property|Construction", na=False
                    )
                ]
            )
    ...
    def merge_and_finalize(self) -> bool:
        ...
        if self.wards_gdf is not None and self.ward_name_col is not None:
            ...
            self.final_df = self.final_df.merge(
                ward_info, left_on="ward_name", right_on=self.ward_name_col, how="left"
            )
    ...
    def _create_target_variables(self):
        ...
        self.final_df["risk_score"] = (
            self.final_df["target_blight_requests"] / max_blight
            if max_blight > 0
            else 0
        )

        print(
            f"   ✓ High-risk wards (75th percentile): {self.final_df['is_high_blight_risk'].sum()}"
        )
        print(
            f"   ✓ LightGBM target 'is_blighted': {self.final_df['is_blighted'].sum()}"
        )
        print(
            f"   ✓ Extreme-risk wards (90th percentile): {self.final_df['is_extreme_blight_risk'].sum()}"
        )
    ...
    def _add_feature_metadata(self):
        ...
        self.feature_metadata = {
            "volume_features": [
                ...
                if col.startswith("total_") or col.startswith("requests_")
            ],
            "blight_features": [
                col
                for col in feature_cols
                if "blight" in col and not col.endswith("_category")
            ],
            "temporal_features": [
                col for col in feature_cols if col.startswith("temporal_")
            ],
            "trend_features": [col for col in feature_cols if col.startswith("trend_")],
            "seasonal_features": [
                col for col in feature_cols if col.startswith("seasonal_")
            ],
            "ratio_features": [col for col in feature_cols if col.startswith("ratio_")],
            "statistical_features": [
                col for col in feature_cols if col.startswith("stats_")
            ],
            "diversity_features": [
                col for col in feature_cols if "diversity" in col or "dominance" in col
            ],
            "categorical_features": [
                col
                for col in feature_cols
                if col.endswith("_category")
                or col.endswith("_blight")
                and col != "is_blighted"
            ],
            "target_features": [
                col
    ...
    def _validate_features(self):
        ...
        if missing_counts.sum() > 0:
            print(
                f"   ⚠️  Missing values found in {(missing_counts > 0).sum()} features"
            )
        ...
        if len(numeric_cols) > 0:
            ...
            if inf_counts.sum() > 0:
                print(
                    f"   ⚠️  Infinite values found in {(inf_counts > 0).sum()} numeric features"
                )
                # Replace inf with large finite values
                self.final_df[numeric_cols] = self.final_df[numeric_cols].replace(
    ...
    def save_results(self) -> bool:
        ...
        metadata = {
            ...
            "data_summary": {
                ...
                "extreme_risk_wards": int(
                    self.final_df["is_extreme_blight_risk"].sum()
                ),
    ...
    def print_summary(self):
        ...
        print(
            f"   • Training period: {self.config['history_start_year']}-{self.config['history_end_year']}"
        )
        print(f"   • Target year: {self.config['target_year']}")
        print(
            f"   • Total features: {len([col for col in self.final_df.columns if col not in ['ward_name', self.ward_name_col if hasattr(self, 'ward_name_col') else None, 'geometry']])}"
        )
        print(
            f"   • Geometry included: {'Yes' if 'geometry' in self.final_df.columns else 'No'}"
        )

        # Target variable distribution
        print("\n🎯 Target Variable Distribution:")
        print(
            f"   • High-risk wards: {self.final_df['is_high_blight_risk'].sum()}/{len(self.final_df)} ({self.final_df['is_high_blight_risk'].mean():.1%})"
        )
        print(
            f"   • Extreme-risk wards: {self.final_df['is_extreme_blight_risk'].sum()}/{len(self.final_df)} ({self.final_df['is_extreme_blight_risk'].mean():.1%})"
        )
        ...
        print(
            "   🎯 Alternative targets: 'is_high_blight_risk', 'risk_level', 'risk_score'"
        )
        print(
            f"   🏷️  Categorical features: {len(self.feature_metadata.get('categorical_features', []))} features for LightGBM"
        )
        print(
            "   🎯 Use GeoJSON directly with LightGBM model for geometry-based features!"
        )
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print(
            "   🤖 Ready for LightGBM model with geometry features and categorical variables!"
        )
        print(
            "   📊 Includes 'is_blighted' target and categorical features for optimal performance!"
        )
        print(
            "   🗺️  GeoJSON output: 'model_ready_data.geojson' ready for LightGBM model!"
        )
